...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
    ...
    for (auto &kp : keypoints) {
        ...
        if(u - half_patch_size < 0 || v - half_patch_size < 0 
                || u + half_patch_size > image.cols || v + half_patch_size > image.rows) continue;
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints){
        ...
        for (int i = 0; i < 256; i++){
            ...
            if(up1<0 || up1>=image.cols || 
                vp1<0 || vp1>=image.rows || 
                uq1<0 || uq1>=image.cols || 
                vq1<0 || vq1>=image.rows){
                d.clear();
                break;
            }
            else
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for(auto &des1 : desc1){
        ...
        for(auto &des2 : desc2){
            ...
            for(int i = 0; i < 256 ; i++) hammingDistance += (des1[i] == des2[i]) ? 0 : 1;
            des1_match[des2_idx] = hammingDistance;
        }
        auto best_match = *min_element(des1_match.begin(), des1_match.end(),
                            [](const pair<int, int>& p1, const pair<int, int>& p2) {
                                return p1.second < p2.second;});
        
        if(des1_match[best_match.first] < d_max) {
            cv::DMatch m;
            m.queryIdx = des1_idx;
            m.trainIdx = best_match.first;
            m.distance = best_match.second;
            matches.push_back(m);
        }
    }
    // END YOUR CODE HERE
