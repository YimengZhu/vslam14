...
public:
    ...
    EdgeDirectProjection(float *color, cv::Mat &target) {
        ...
        this->width = targetImg.cols;
        this->height = targetImg.rows;
    }
    ...
    virtual void computeError() override {
        ...
        if(u-3 < 0 || u + 2 > width || v-3 < 0 || v + 2 > height) {
    	    this->setLevel(1);
...
int main(int argc, char **argv) {
    ...
    boost::format fmt("./images/%d.png");
    for (int i = 0; i < 7; i++) {
        images.push_back(cv::imread((fmt % i).str(), 0));
    }
    cout << "image reading finished"<< endl;
    ...
    cout << "optimizer set up done " << endl;
    ...
    // END YOUR CODE HERE
    cout << "vertex and edge set up done." << endl;
    ...
    cout << "fecthed data from optimizer"<<endl;
